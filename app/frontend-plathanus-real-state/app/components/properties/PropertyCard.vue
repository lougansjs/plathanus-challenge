<!-- app/components/properties/PropertyCard.vue -->
<script setup lang="ts">
import type { Property } from '../../../types/property'
import { useMoney } from '../../composables/useMoney'
import { usePropertyStatus } from '../../composables/usePropertyStatus'

// @ts-ignore - defineProps is auto-imported by Nuxt
const props = defineProps<{ item: Property }>()
const { brl } = useMoney()
const { getStatusLabel, getStatusColor, getContractTypeLabel } = usePropertyStatus()

const formatDate = (dateString: string) => {
  const date = new Date(dateString)
  return date.toLocaleDateString('pt-BR', { day: 'numeric', month: 'short' })
}
</script>

<template>
  <NuxtLink
    :to="`/properties/${item.id}`"
    class="block bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden"
  >
    <!-- Image with Status Badge -->
    <div class="relative h-48 overflow-hidden">
      <img
        :src="item.cover_photo?.card_url || item.photos?.[0]?.card_url || '/placeholder-property.jpg'"
        class="w-full h-full object-cover"
        alt="cover"
      />
      <!-- Status Badge -->
      <div class="absolute top-4 left-4">
        <span
          class="px-3 py-1 rounded-full text-xs font-semibold"
          :class="getStatusColor(item.status).bg + ' ' + getStatusColor(item.status).text"
        >
          {{ getStatusLabel(item.status) }}
        </span>
      </div>
      <!-- Code Overlay -->
      <div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white p-3">
        <div class="font-semibold">{{ item.name }}</div>
      </div>
    </div>

    <!-- Content -->
    <div class="p-4 space-y-3">
      <h3 class="font-semibold text-gray-900">{{ item.name }}</h3>
      
      <!-- Location -->
      <div class="flex items-center gap-1 text-sm text-gray-600">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
        <span>{{ item.address?.neighborhood || item.address?.city || 'Localização não informada' }}</span>
      </div>

      <!-- Features -->
      <div class="flex items-center gap-4 text-sm text-gray-600">
        <div class="flex items-center gap-1">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
          </svg>
          <span>{{ item.rooms }}</span>
        </div>
        <div class="flex items-center gap-1">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10h16v11H4V10z" />
          </svg>
          <span>{{ item.bathrooms }}</span>
        </div>
        <div class="flex items-center gap-1">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" />
          </svg>
          <span>{{ item.area || 0 }}m²</span>
        </div>
      </div>

      <!-- Price -->
      <div class="flex items-center justify-between pt-2 border-t">
        <div>
          <p v-if="item.price" class="text-green-600 font-bold text-lg">
            {{ brl(item.price) }}/{{ getContractTypeLabel(item.contract_type) === 'Aluguel' ? 'mês' : 'venda' }}
          </p>
          <p v-else class="text-gray-400 text-sm">Preço não informado</p>
        </div>
        <p v-if="item.created_at" class="text-xs text-gray-500">
          Disponível {{ formatDate(item.created_at) }}
        </p>
      </div>
    </div>
  </NuxtLink>
</template>

